import type { CarePlan, LifestyleAction, Medication } from "@/lib/types";
  ...
  return (
    <div className="px-5 pb-8 pt-4" style={{ fontFamily: "var(--font-inter), sans-serif" }}>
      <header className="reveal" style={delay(0)}>
        ...
        <h1 className="mt-1 text-[28px] font-semibold text-ink2" style={{ fontFamily: "var(--font-sora), sans-serif" }}>
              ...
              <div key={m.id} className="mono-card overflow-hidden rounded-3xl">
                <div className="flex items-center gap-3.5 p-2.5">
                  ...
                  >
                    ...
                    <img src={`/objects/${o.obj}`} alt="" className="h-[88px] w-[88px] object-contain mix-blend-multiply" />
                  </span>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-[16.5px] font-semibold text-ink2" style={{ fontFamily: "var(--font-sora), sans-serif" }}>
                        {m.name}
                      </span>
                      <span className="rounded-full px-2 py-0.5 text-[11px] font-semibold" style={{ background: st.bg, color: st.fg }}>
                        {st.label}
                      </span>
                    </div>
          ...
          {plan.lifestyleActions.map((a) => {
            ...
            return (
              <div key={a.id} className="mono-card flex items-center gap-3.5 rounded-3xl p-2.5">
                <span className="grid h-14 w-14 shrink-0 place-items-center overflow-hidden rounded-2xl" style={{ background: o.tint }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={`/objects/${o.obj}`} alt="" className="h-[64px] w-[64px] object-contain mix-blend-multiply" />
                </span>
      ...
      <Section index={4} title="When to get help">
        <div className="space-y-2.5">
          {plan.redFlags.map((f) => (
            <div key={f.id} className="rounded-3xl border border-[rgba(224,113,79,0.28)] bg-[#fbeadf] p-4">
              <div className="flex items-start gap-2">
                <span className="mt-1.5 h-2 w-2 shrink-0 rounded-full" style={{ background: "#e0714f" }} />
                <div>
...
function Section({
  ...
  return (
    <section className="reveal mt-6" style={delay(index)}>
      <h2 className="mb-3 text-[17px] font-semibold text-ink2" style={{ fontFamily: "var(--font-sora), sans-serif" }}>
        {title}
      </h2>
      {children}
    </section>
