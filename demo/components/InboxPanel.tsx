import type { CheckInSeverity, InboxItem } from "@/lib/types";

const SEVERITY_STYLE: Record<CheckInSeverity, string> = {
  mild: "bg-care-wash text-care-strong",
      ...
      <div className="divide-y divide-line">
        {items.length === 0 ? (
          <p className="px-5 py-8 text-center text-[14px] text-muted">
            Nothing here yet. When Meera logs how she&apos;s feeling and it
            crosses a threshold, it appears here with context.
          </p>
        ) : (
          items.map((item) => {
            const isGlucose = item.kind === "glucose";
            const title = isGlucose
              ? `Glucose ${item.reading?.value} mmol/L`
              : item.checkIn?.symptom;
            const tag = isGlucose
              ? item.reading?.context
              : item.checkIn?.severity;
            const tagStyle = isGlucose
              ? "bg-clay-wash text-clay"
              : SEVERITY_STYLE[item.checkIn?.severity ?? "mild"];
            const loggedAt = isGlucose
              ? item.reading?.loggedAt
              : item.checkIn?.loggedAt;
            return (
              <div
                key={item.id}
                className={`px-5 py-4 ${item.read ? "opacity-55" : ""}`}
              >
                <div className="flex items-center justify-between">
                  ...
                  {!item.read ? (
                    <button
                      ...
                      className="text-[13px] font-medium text-care hover:text-care-strong"
                    >
                      Mark read
                    </button>
                  ) : null}
