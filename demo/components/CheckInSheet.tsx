...
function Face({ level, active }: { level: CheckInSeverity; active: boolean }) {
  ...
  return (
    <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke={stroke} strokeWidth="1.8" strokeLinecap="round">
  ...
  return (
    ...
    >
      <div className="rounded-t-[28px] border-t border-white bg-white p-5 pb-6 shadow-[0_-24px_50px_-20px_rgba(23,26,31,0.35)]">
        ...
        <h3 className="text-[22px] font-semibold text-ink2" style={{ fontFamily: "var(--font-sora), sans-serif" }}>
          How are you feeling?
        </h3>
            ...
            return (
              <button
                ...
                className={`rounded-full px-3.5 py-2 text-[14px] font-medium transition ${
                  on
                    ? "bg-mint text-ink2"
                    : "bg-canvas text-slate hover:text-ink2"
                }`}
              >
                {s}
              </button>
          ...
          {SEVERITIES.map((s) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-[13px] font-semibold ${on ? "text-mint-strong" : "text-slate"}`}>
                  {s.label}
                </span>
              </button>
            );
